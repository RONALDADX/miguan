...
class Strategy01WeeklyExecutor:
    ...
    def check_strategy_01_conditions(self, indicators: Dict) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            # 条件6：DPO角度>20度
            dpo_angle_gt_20 = bool(dpo_info.get('angle_gt_20', False))
            
            conditions['dpo_angle'] = {
                'name': 'DPO角度>20度',
                'met': dpo_angle_gt_20,
                'details': {
                    'dpo_angle': float(dpo_info.get('angle', 0)),
            ...
            obv_above_ma_5days = bool(obv_info.get('above_ma_5days', False))
        ...
        if results['triggered_stocks']:
            ...
            for i, stock in enumerate(triggered_stocks, 1):
                ...
                report_lines.append(
                    ...
                    f"{met_count}/12{'':<4} {key_indicators_str:<30}"
                )
            ...
            if failed_stocks:
                ...
                for i, stock in enumerate(failed_stocks[:10], 1):
                    ...
                    report_lines.append(f"  {i:2d}. {symbol} - {name}: 满足{met_count}/12个条件")
        ...
        if results['triggered_stocks']:
            ...
            if failed_stocks:
                ...
                for stock in failed_stocks[:5]:
                    print(f"    • {stock['symbol']} - {stock['name']}: 满足{stock.get('met_conditions_count', 0)}/12个条件")
